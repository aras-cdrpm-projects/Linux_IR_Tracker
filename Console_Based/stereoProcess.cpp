...
void stereoProcess::printInfo()
{
    ...
    qDebug() << "Size of map Matrix is:" << mapLeftX.cols << ',' << mapLeftY.rows;
    qDebug() << "Left Location 200,300 will be maped to"<<mapLeftX.at<float>(200,300) <<','<< mapLeftY.at<float>(200,300);
    qDebug() << "Right Location 200 ,200 will be maped to"<<mapRightX.at<float>(300,200) <<','<< mapRightY.at<float>(300,200);
    //qDebug()<< lK1 << lK2 << lK3 << lP1 << lP2;
    qDebug() << "Using the inverse function, Point: 200,300 will be maped to:"<<dx<<','<<dy;
    qDebug()<<"Nonlinear Solver Problem ...";
    double x,y,X,Y,aa,bb;
    x=0.1,y=0.3;
    forwardDis(x,y,&X,&Y,1);
    qDebug() << "input to forwardDist is:"<<x<<','<<y <<"and The Output is:"<<X<<','<<Y<<" Vertified Ok!";
    roundF(0.2,0.3,&X,&Y,1);
    qDebug() << "input to roundF is 0.2 , 0.3 and the output is(rf1,rf2):"<<X<<','<<Y << " Vertified Ok!";

    forwardDis(x,y,&X,&Y,1);
    solverGD(X,Y,&aa,&bb,1);
    qDebug() << "Input 0.1 , 0.3 was applied to forwardDis and it's output is fed to solver GD, the solution is expected to be the same as Input 0.1 , 0.3 the resault is:"<<aa<<','<<bb;

    inverseRectify(200 , 300 , &dx,&dy,CAM_LEFT);
    forwardMap(dx,dy,&X,&Y,CAM_LEFT);
    qDebug() <<"Right Point X 200 , 300 is Remapping to"<<X<<','<<Y;
...
void stereoProcess::inverseRectify(double sX, double sY, double *dX, double *dY , int Cam)
{
    ...
    if(Cam == CAM_LEFT){
        ...
        *dY=rectPointRight.at<double>(1,0);
        ...
        qDebug() << "Error Inverse Rectifie unknown Camera";
        *dX=-1;
        *dY=-1;
